...
export const Modal: React.FC<ModalProps> = ({ children, title, isOpen, ...props }) => {
	function handleCloseClick(e: React.MouseEvent) {
		const { target } = e;
		const modalWrapper = (target as HTMLElement).getAttribute('data-modal-wrapper');
		const modalCloseButton = (target as HTMLElement).getAttribute('data-modal-close');
		const modalCancelButton = (target as HTMLElement).getAttribute('data-modal-cancel');

		if(modalWrapper || modalCloseButton || modalCancelButton) props.onClose?.('click', e.target);
	}
